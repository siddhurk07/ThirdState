...
public class CollectibleScript9 : MonoBehaviour {
	public static int j;
	...
	public IEnumerator Down()
	{
		for (int j=0; j<28; j++) 
		{
			if (collections.transform.position == pivots [j].transform.position) 
			{
				...
				{
					collections.layer = 8;
					collections.gameObject.tag = "ob5";
				}
				if(j<4)
				{
					while(true)
				...
				if(Destruct.GameValue > 60 && (j==11 || j==15 || j==19 || j==23 || j==27))
				{
					while(true)
					{
				...
				if(Destruct.GameValue > 60 &&  j!=27 && j>7)
				{
					while(true)
					{
